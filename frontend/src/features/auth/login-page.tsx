    ...
    <div className="relative mx-auto grid min-h-[calc(100vh-2rem)] max-w-[1440px] overflow-hidden rounded-[28px] border border-white/70 bg-white/65 shadow-floating backdrop-blur-sm lg:grid-cols-[1.15fr_.85fr]">
      ...
      <section className="flex min-h-full items-center justify-center p-6 sm:p-12 lg:p-16">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }} className="w-full max-w-[400px]">
          ...
          <div className="mb-9"><p className="text-sm font-semibold text-blue-600">WELCOME BACK</p><h2 className="heading mt-2 text-3xl font-extrabold tracking-tight text-slate-900">Sign in to TaskFlow</h2><p className="mt-2 text-sm leading-6 text-slate-500">Enter your details to access your workspace.</p></div>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
            ...
            <Button type="submit" loading={isSubmitting} className="w-full">Continue <ArrowRight size={17} /></Button>
          </form>
          <p className="mt-8 text-center text-sm text-slate-500">New to TaskFlow? <Link to="/signup" className="font-semibold text-blue-600 hover:text-blue-700">Create an account</Link></p>
        </motion.div>
      </section>
    </div>
  </main>
